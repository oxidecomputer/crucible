...
pub(crate) mod test {
    ...
    fn work_errors_are_counted(is_write_unwritten: bool) {
        ...
        // Before the last response, the write should be fast-acked, but
        // write_unwritten is not.  In either case, there is no ackable work.
        assert_eq!(ds.ackable_work.len(), 0);
        assert_eq!(
            ds.ds_active.get(&next_id).unwrap().acked,
            !is_write_unwritten
        );
        ...
        // After the last response, write_unwritten should be marked as
        // ackable.  write should not, because it must be fast-acked
        assert_eq!(
            ds.ackable_work.len(),
            if is_write_unwritten { 1 } else { 0 }
        );
    ...
    fn work_delay_completion_flush(is_write_unwritten: bool) {
        ...
        // Both writes can now ACK to the guest (or have been fast-acked)
    ...
    fn work_completed_writeio_flush(is_write_unwritten: bool) {
        ...
        // Ack the write to the guest, or check that it should be fast-acked
        if is_write_unwritten {
            ds.ack(next_id);
        } else {
            assert!(ds.ds_active.get(&next_id).unwrap().acked);
    ...
    fn work_delay_completion_flush_order(is_write_unwritten: bool) {
        ...
        // Ack the writes to the guest, or check for fast-ack
        if is_write_unwritten {
            ds.ack(id1);
            ds.ack(id2);
        } else {
    ...
    fn work_completed_write_ack_ready_replay(is_write_unwritten: bool) {
        ...
        // Ack this job immediately, or verify that it was fast-acked
        if is_write_unwritten {
            ds.ack(id1);
        } else {
            assert!(ds.ds_active.get(&id1).unwrap().acked);
    ...
    fn test_repair_dep_cleanup_some() {
        ...
        assert_eq!(
            ds.in_progress(JobId(1006), ClientId::new(1))
                .unwrap()
                .deps(),
            &vec![]
        );
        ...
        assert_eq!(
            ds.in_progress(JobId(1007), ClientId::new(1))
                .unwrap()
                .deps(),
            &vec![JobId(1006)]
        );
        ...
        assert_eq!(
            ds.in_progress(JobId(1008), ClientId::new(1))
                .unwrap()
                .deps(),
            &vec![JobId(1007)]
        );
    ...
    fn test_repair_dep_cleanup_repair() {
        ...
        assert!(ds.in_progress(JobId(1007), ClientId::new(1)).is_none());
        ...
        assert_eq!(
            ds.in_progress(JobId(1013), ClientId::new(1))
                .unwrap()
                .deps(),
            &vec![JobId(1006), JobId(1008), JobId(1012)]
        );
        assert_eq!(current_deps, &[JobId(1006), JobId(1008), JobId(1012)]);
    }
    ...
    fn test_repair_dep_cleanup_sk_repair() {
        ...
        assert_eq!(
            ds.in_progress(JobId(flushclose_jobid), ClientId::new(0))
                .unwrap()
                .deps(),
            &vec![JobId(1000)]
        );
        assert_eq!(
            ds.in_progress(JobId(flushclose_jobid), ClientId::new(1))
                .unwrap()
                .deps(),
            &vec![]
        );
        assert_eq!(
            ds.in_progress(JobId(flushclose_jobid), ClientId::new(2))
                .unwrap()
                .deps(),
            &vec![JobId(1000)]
        );
        ...
        let write_job = ds.ds_active.get(&write_jobid).unwrap();
        assert_eq!(write_job.state[ClientId::new(0)], IOState::New);
        assert_eq!(write_job.state[ClientId::new(1)], IOState::New);
        assert_eq!(write_job.state[ClientId::new(2)], IOState::New);

        assert_eq!(
            ds.in_progress(write_jobid, ClientId::new(0))
                .unwrap()
                .deps(),
            &vec![JobId(1004)]
        );
        assert_eq!(
            ds.in_progress(write_jobid, ClientId::new(1))
                .unwrap()
                .deps(),
            &vec![JobId(1004)]
        );
        assert_eq!(
            ds.in_progress(write_jobid, ClientId::new(2))
                .unwrap()
                .deps(),
            &vec![JobId(1004)]
        );
