...
pub(crate) mod up_test {
    ...
    async fn _replace_downstairs_basic() {
        // Replace a downstairs, see what happens.
        let up_uuid = Uuid::new_v4();
        let d0 = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 1234);
        let d1 = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 5678);
        let d2 = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 9012);
        let d3 = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 1314);

        let mut ddef = RegionDefinition::default();
        ddef.set_block_size(512);
        ddef.set_extent_size(Block::new_512(100));
        ...
        let up =
            Upstairs::new(&opts, 0, Some(ddef), guest.clone(), log.clone());
        // let (ds_done_tx, _ds_done_rx) = mpsc::channel(500);
        for cid in 0..3 {
            up.ds_transition(cid, DsState::WaitActive).await;
            up.ds_transition(cid, DsState::WaitQuorum).await;
        ...
        info!(log, "going active");
        up.set_active().await.unwrap();

        info!(log, "We are active, wait.");
        tokio::time::sleep(Duration::from_secs(3)).await;
        let res = guest.replace_downstairs(up_uuid, d0, d3).await;
        info!(log, "Returned {:?}", res);

        /*

        // Create a write
        let first_id = enqueue_write(&up, true, ds_done_tx.clone()).await;
        ...
        // Process the operation for client 0
        assert!(!up
            .process_ds_operation(first_id, 0, ok_response.clone(), None)
            .await
            .unwrap(),);

        // process_ds_operation should return true after we process this.
        assert!(up
            .process_ds_operation(first_id, 1, ok_response.clone(), None)
            .await
            .unwrap());

        // Process client 2
        assert!(!up
            .process_ds_operation(first_id, 2, ok_response.clone(), None)
            .await
            .unwrap(),);
        ...
        let request = crucible_protocol::ReplaceDownstairs {
            id: Uuid::new_v4(),
            old,
            new,
        };
